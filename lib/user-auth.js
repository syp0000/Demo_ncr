...
export function isEnvAdminUser(userOrId) {
  const id = typeof userOrId === 'object'
    ? userOrId?.id
    : userOrId;
  const envUser = getEnvUserById(id);
  return normalizeRole(envUser?.role) === 'admin';
}
...
export function getPublicUserAuthConfig() {
  return {
...
export async function ensureUserAuthSchema() {
  ...
  await Promise.all([
    query(`CREATE INDEX IF NOT EXISTS idx_signup_requests_status ON user_signup_requests (status, requested_at DESC)`),
    query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_signup_requests_unique_pending ON user_signup_requests (LOWER(username)) WHERE status = 'pending'`),
  ]);
  schemaEnsured = true;
}
...
export async function getUserById(id) {
  ...
  const envUser = getEnvUserById(normalizedId);
  if (envUser) {
    return {
      id: envUser.id,
  ...
  return getDbUserById(normalizedId);
}
